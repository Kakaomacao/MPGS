...
def save_pcd2ply(outdir, pts3d, colors, filename='output.ply'):
    ...
    ply_path = os.path.join(outdir, filename)
...
def get_3D_model_from_scene(outdir, silent, scene, min_conf_thr=3, as_pointcloud=False, mask_sky=False,
                            clean_depth=False, transparent_cams=False, cam_size=0.05, selected_images=None):
    """
    ...
    if selected_images is not None:
        ...
        for i in selected_images:
            pts3d.append(to_numpy(scene.get_pts3d()[i]))
            scene.min_conf_thr = float(scene.conf_trf(torch.tensor(min_conf_thr)))
            msk.append(to_numpy(scene.get_masks()[i]))
            rgbimg_filtered.append(to_numpy(rgbimg[i]))
        rgbimg = rgbimg_filtered
        filename = 'output_selected0.ply'
    else:
    ...
    if as_pointcloud:
        ...
        ply_path = save_pcd2ply(os.path.join(outdir), pts.reshape(-1, 3), col.reshape(-1, 3),
                                filename=filename)
