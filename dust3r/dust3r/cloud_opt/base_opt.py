...
class BasePCOptimizer (nn.Module):
    ...
    def get_masks(self, threshold=None):
        # threshold 값 설정
        threshold = threshold if threshold is not None else self.min_conf_thr
        # threshold 값을 기준으로 마스크 생성
        return [(conf > threshold) for conf in self.im_conf]
    
    def get_masks_depth(self):
        cams = inv(self.get_im_poses())
        K = self.get_intrinsics()
        depthmaps = self.get_depthmaps()
        res_im_conf = []
        for param in self.im_conf:
            cloned_param = param.clone().detach()
            res_im_conf.append(cloned_param)

        for i, pts3d in enumerate(self.depth_to_pts3d()):
            msk_i = (depthmaps[i] > 0) 
            threshold_depth = torch.median(depthmaps[i]) * 2.0
            # retain all the background information
            back_msk_i = (depthmaps[i] > threshold_depth)
            random_mask = torch.rand(back_msk_i.size()) < 1
            back_msk_i = back_msk_i & random_mask.to("cuda")
            res_im_conf[i] += (5 * back_msk_i)
        return [(conf > self.min_conf_thr) for conf in res_im_conf]
