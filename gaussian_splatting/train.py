    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        with torch.no_grad():
            ...
            if iteration % 1000 == 0:
                import cv2
                import numpy as np
                full_view = scene.getTrainCameras().copy()
                p = f"/home/airlabs/SuGaR/test_folder/{iteration}"
                ...
                for v in full_view:
                    with torch.no_grad():
                        render_pkg = render(v, gaussians, pipe, background)
                        image, viewspace_point_tensor, visibility_filter, radii = render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"]
                        gt_image = v.original_image.cuda()
                        
                        # Rendered image 처리 및 저장
                        rendered_image = (image * 255).detach().cpu().numpy().transpose(1, 2, 0).astype(np.uint8)
                        rendered_image_bgr = cv2.cvtColor(rendered_image, cv2.COLOR_RGB2BGR)  # RGB -> BGR
                        cv2.imwrite(f"{p}/{v.uid}_rendered.png", rendered_image_bgr)

                        # Ground Truth image 처리 및 저장
                        gt_image_np = (gt_image * 255).detach().cpu().numpy().transpose(1, 2, 0).astype(np.uint8)
                        gt_image_bgr = cv2.cvtColor(gt_image_np, cv2.COLOR_RGB2BGR)  # RGB -> BGR
                        cv2.imwrite(f"{p}/{v.uid}_gt.png", gt_image_bgr)
    
    # TODO: optimizing parameter 저장
...
def prepare_output_and_logger(args):    
    if not args.model_path:
        ...
        args.model_path = os.path.join("./output/", args.dataset, unique_str[0:10])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[3000, 6_000, 10_000, 15_000, 20_000, 25_000, 30_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[3000, 6_000, 10_000, 15_000, 20_000, 25_000, 30_000])
