...
class LooDataset(Dataset):
    def __init__(self, cfg: LooDataModuleConfig, split: str = 'train', sparse_num: int = 0):
        ...
        images_folder=os.path.join(self.data_dir, 'dust3r', 'images')
        ...
        if cfg.use_dust3r:
            if len(cfg.json_path):
                ...
                with open(os.path.join(self.data_dir, f"dust3r_{self.sparse_num}.json")) as f:
                    dust3r_frames = json.load(f)
            dust3r_frames = [dust3r_frames[i] for i in [26, 25, 27]]
            ...
            depth_path = os.path.join(self.data_dir, "dust3r", "depths")
            depth_files = sorted([f for f in os.listdir(depth_path) if f.endswith(".png")])
            depth_maps = []
            for file in depth_files:
                img_path = os.path.join(depth_path, file)
                depth_map = cv2.imread(img_path, cv2.IMREAD_UNCHANGED)  # 원본 깊이 값 유지
                depth_maps.append(depth_map)
            
            dust3r_depths = np.array(depth_maps)
