...
class GaussianDreamer(BaseLift3DSystem):
        ...
        if hasattr(viewpoint_cam, "mono_depth") and viewpoint_cam.mono_depth is not None:
            if mono_loss_type == "mid":
                ...
                if mask.sum() < 10:
                    ...
                    disp_render = 1 / render_pkg["depth"][0][mask].clamp(1e-6) # shape: [N]
                ...
                rendered_depth = render_pkg["depth"][0][viewpoint_cam.mask > 0.5].clamp(1e-6)
    ...
    def render_gs(self, batch: Dict[str, Any], renderbackground=None, need_loss=False) -> Dict[str, Any]:
        ...
        for id in range(batch['index'].shape[0]):
            ...
            depths.append(render_pkg["depth"][0])
    ...
    def on_before_optimizer_step(self, optimizer):
        with torch.no_grad():
            if self.true_global_step < self.opt.densify_until_iter:
                ...
                # if self.gaussian.max_radii2D.device == torch.device("cpu"):
                #     self.gaussian.max_radii2D.to("cuda")
                if len(self.gaussian.max_radii2D) == 0:
                    self.gaussian.max_radii2D = torch.zeros_like(self.radii)
