...
def leave_one_out_training(args, dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, train_id):
    opt.iterations = 3000
    opt.densify_until_iter = int(0.4 * opt.iterations)
    
    first_iter = 2000 # in this code, we just use the data from 6000 iter
...
def cal_loss(opt, args, image, render_pkg, viewpoint_cam, bg, silhouette_loss_type="bce", mono_loss_type="mid", iteration=0):
    ...
    #[FIXME]----------------------------------------------------------------------------------------
    #tmp mask
    if viewpoint_cam.tmask is None:
        viewpoint_cam.tmask = torch.ones_like(viewpoint_cam.original_image[0], device="cuda").unsqueeze(0)
    #[FIXME]----------------------------------------------------------------------------------------

    gt_image = viewpoint_cam.original_image.to(image.dtype).cuda()
    if opt.random_background:
        gt_image = gt_image * viewpoint_cam.tmask + bg[:, None, None] * (1 - viewpoint_cam.tmask).squeeze()
    Ll1 = l1_loss(image, gt_image)
    loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image))

    if hasattr(args, "use_mask") and args.use_mask:
        if silhouette_loss_type == "bce":
            silhouette_loss = F.binary_cross_entropy(render_pkg["rendered_alpha"], viewpoint_cam.tmask)
        elif silhouette_loss_type == "mse":
            silhouette_loss = F.mse_loss(render_pkg["rendered_alpha"], viewpoint_cam.tmask)
        else:
            raise NotImplementedError
        loss = loss + opt.lambda_silhouette * silhouette_loss
        
    if hasattr(viewpoint_cam, "mono_depth") and  viewpoint_cam.depth is not None:
        if isinstance(viewpoint_cam.depth, np.ndarray):
            viewpoint_cam.depth = torch.from_numpy(viewpoint_cam.depth).to("cuda").unsqueeze(0)
        if mono_loss_type == "mid":
            # we apply masked monocular loss
            gt_mask = torch.where(viewpoint_cam.tmask > 0.5, True, False)
            render_mask = torch.where(render_pkg["rendered_alpha"] > 0.5, True, False)
            mask = torch.logical_and(gt_mask, render_mask)
            if mask.sum() < 10:
                depth_loss = 0.0
            else:
                disp_mono = 1 / viewpoint_cam.depth[mask].clamp(1e-6) # shape: [N]
                disp_render = 1 / render_pkg["depth"][mask].clamp(1e-6) # shape: [N]
                depth_loss = monodisp(disp_mono, disp_render, 'l1')[-1]
        elif mono_loss_type == "pearson":
            zoe_depth = viewpoint_cam.depth[viewpoint_cam.tmask > 0.5].clamp(1e-6)
            rendered_depth = render_pkg["depth"][viewpoint_cam.tmask > 0.5].clamp(1e-6)
            depth_loss = min(
...
def train_3dgs(args, ids):
    ...
    for num_id, image_id in zip(range(args.sparse_view_num), ids): # num_id: leave one out id, image_id: the id of the image to be infered
        ...
        args.start_checkpoint = os.path.join(args.model_path, 'chkpnt2000.pth') # load this ckpt
        # os.makedirs(args.model_path, exist_ok=True)
...
if __name__ == "__main__":
    ...
    args.resolution = 1
    args.sparse_view_num = 3
    args.sh_degree = 3
    # args.init_pcd_name = "dust3r_3"
    # args.dust3r_json = "output/gs_init/scan8/refined_cams.json"
    args.white_background = True
    args.render_path = True
    # args.random_background = True
    args.use_dust3r = True
    args.images = "images"
    args.eval = True
